...
public class PrintDialog extends JDialog implements ActionListener, DocumentListener, PropertyChangeListener, Printable {
	...
	public PrintDialog(JFrame frame, String title, boolean modal, PrintPanel cap) {
	    ...
	    ratioList = new JComboBox(ratioSizes);
...
//		if ( ( ( p.getOrientation() == PageFormat.PORTRAIT ) && ( orientationList.getSelectedItem() == this.LANDSCAPE ) )
//			|| ( ( p.getOrientation() == PageFormat.LANDSCAPE ) && ( orientationList.getSelectedItem() == this.PORTRAIT ) ) 
//			|| ( ( p.getOrientation() == PageFormat.REVERSE_LANDSCAPE ) && ( orientationList.getSelectedItem() == this.PORTRAIT ) ) ) {
//			bufImgPrint = rotateImage(bufImgPrint);
//		}
...
//	private BufferedImage rotateImage(BufferedImage bufImgPrint) {
//		BufferedImage rotImage = new BufferedImage(bufImgPrint.getHeight(), bufImgPrint.getWidth(), BufferedImage.TYPE_INT_RGB);
//		Graphics2D g2 = (Graphics2D)rotImage.getGraphics();
//		g2.rotate(Math.PI/2);
//    	g2.drawImage(bufImgPrint, 0, -bufImgPrint.getHeight(), null);
// 		g2.dispose();
//		return rotImage;
//	}
