...
public class DynoHistogram extends View {
	...
	private int rightImageBoxWidth = 0;
	private int rightImageBoxHeight = 0;
	...
	private final int leftBoxWidth = 290;  //fixed.  Should not change.
	...
	protected void paintComponent(Graphics g) {
		...
		for (int x=0;x<imageViewCol.size();x++) {
			...
			if ( !settings.FilterState || (settings.FilterState && !IV.imageAnalysis.Hidden ) )  {
				...
				if (( imgSelected >= smImgPosition ) && ( imgDisplayed < imgsPerPhySliderWidth ) ) {
					...
					//g.draw3DRect(imgBoxX1-1, imgBoxY1-1, smImgBoxWidth+2, smImgBoxHeight+2, true);
		...
		if ( selectedImgView.imageAnalysis != null ) {
			...
			int Rotation_Loc2 = Rotation_Loc + 40;
			...
			int H_S_Flag_Loc = Rotation_Loc2 + 16;
			int H_S_Flag_Loc2 = H_S_Flag_Loc + 68;
			...
			int Grid_3rds_Loc = H_S_Flag_Loc2 + 16;
			int Grid_3rds_Loc2 = Grid_3rds_Loc + 45;
			...
			int Zoom_Loc = Grid_3rds_Loc2 + 16;
			int Zoom_Loc2 = Zoom_Loc + 62;
			g.setColor(Color.CYAN);
			g.drawString("[-] Zoom [+]", Zoom_Loc, leftImageBoxY2+14);
			activeRegions.add(new ActiveRegion("Zoom -", ActiveRegion.TEXT_CMD, -1, Zoom_Loc-2, leftImageBoxY2+2, Zoom_Loc + 12, leftImageBoxY2+17 ));
			activeRegions.add(new ActiveRegion("Zoom +", ActiveRegion.TEXT_CMD, -1, Zoom_Loc2-12, leftImageBoxY2+2, Zoom_Loc2 + 2, leftImageBoxY2+17 ));
			...
			activeRegions.add(new ActiveRegion("Brightness", ActiveRegion.TEXT_CMD, -1, brightnessX1, brightnessY2+2, brightnessX1+65, brightnessY2+16 ));

			
			//Set Contrast
			g.setFont(fontBold12);
			g.setColor(Color.CYAN);	
			int contrastX1 = leftImageBoxX1 + 5;
			int contrastY2 = leftImageBoxY2 + 50;
			g.drawString("Contrast", contrastX1, contrastY2+14);
			activeRegions.add(new ActiveRegion("Contrast", ActiveRegion.TEXT_CMD, -1, contrastX1, contrastY2+2, contrastX1+55, contrastY2+16 ));
			

			//Set Hightlights
			g.setFont(fontBold12);
			g.setColor(Color.CYAN);	
			int hightlightX1 = leftImageBoxX1 + 70;
			int hightlightY2 = leftImageBoxY2 + 30;
			g.drawString("Highlights", hightlightX1, hightlightY2+14);
			activeRegions.add(new ActiveRegion("Highlights", ActiveRegion.TEXT_CMD, -1, hightlightX1, hightlightY2+2, hightlightX1+60, hightlightY2+16 ));

			//Set Shadows
			g.setFont(fontBold12);
			g.setColor(Color.CYAN);	
			int shadowsX1 = leftImageBoxX1 + 70;
			int shadowsY2 = leftImageBoxY2 + 50;
			g.drawString("Shadows", shadowsX1, shadowsY2+14);
			activeRegions.add(new ActiveRegion("Shadows", ActiveRegion.TEXT_CMD, -1, shadowsX1, shadowsY2+2, shadowsX1+60, shadowsY2+16 ));
			
			//Set Saturation
			g.setFont(fontBold12);
			g.setColor(Color.CYAN);	
			int saturationX1 = leftImageBoxX1 + 135;
			int saturationY2 = leftImageBoxY2 + 30;
			g.drawString("Saturation", saturationX1, saturationY2+14);
			activeRegions.add(new ActiveRegion("Saturation", ActiveRegion.TEXT_CMD, -1, saturationX1, saturationY2+2, saturationX1+65, saturationY2+16 ));
		
			//Set SharpenBlur
			g.setFont(fontBold12);
			g.setColor(Color.CYAN);	
			int sharpenX1 = leftImageBoxX1 + 135;
			int sharpenY2 = leftImageBoxY2 + 50;
			g.drawString("Sharp/Blur", sharpenX1, sharpenY2+14);
			activeRegions.add(new ActiveRegion("Sharpen/Blur", ActiveRegion.TEXT_CMD, -1, sharpenX1, sharpenY2+2, sharpenX1+65, sharpenY2+16 ));

			//Set Compare
			g.setFont(fontBold12);
			g2.setColor(Color.CYAN);
			int compareX1 = leftImageBoxX1 + 205;
			int compareY2 = leftImageBoxY2 + 30;
			g.drawString("Compare", compareX1, compareY2+14);
			activeRegions.add(new ActiveRegion("Compare", ActiveRegion.TEXT_CMD, -1, compareX1, compareY2+2, compareX1+55, compareY2+16));

			//Set Crop
			g.setFont(fontBold12);
			g2.setColor(Color.CYAN);
			int cropX1 = leftImageBoxX1 + 215;
			int cropY2 = leftImageBoxY2 + 50;
			g.drawString("Crop", cropX1, cropY2+14);
			activeRegions.add(new ActiveRegion("Crop", ActiveRegion.TEXT_CMD, -1, cropX1, cropY2+2, cropX1+35, cropY2+16));
			...
			if ( selectedImgView.ActHist_Flag ) {
				...
				activeRegions.add(new ActiveRegion("Hist_Full", ActiveRegion.TEXT_CMD, -1, HistTitleX1-2, HistTitleY1-12, HistTitleX1+116, HistTitleY1+4 ));
			}else {
				g.drawString("Histograms [Zoom]", HistTitleX1, HistTitleY1);
				activeRegions.add(new ActiveRegion("Hist_Zoom", ActiveRegion.TEXT_CMD, -1, HistTitleX1-2, HistTitleY1-12, HistTitleX1+116, HistTitleY1+4 ));
			}
	...
	class ImageLoader extends SwingWorker<Void, SmImageView> {
		...
		protected Void doInBackground() throws Exception {
			...
			for (int x=nxtID; x<settings.ImageAnalysisCol.size() && !isCancelled(); x++) {
				...
				IV.getScaledImage();
				publish(IV);
				// setProgress(100 * (x+1)/settings.ImageAnalysisCol.size());
			}
			return null;
		}
